...
<div class="users form large-9 medium-8 columns content">
	...
	<?= $this->Form->create($team, array('templates' => $form_template,
		'type' => 'file')); ?>
	<fieldset>
		<legend><?= __('チーム情報編集') ?></legend>
		<?php
		echo $this->Form->control('team_name', array('label' => array('text' => 'チーム名',
			// labelで出力するテキスト
			...
			'templateVars' => array('div_class' => 'form-group row',
				...
				'div_tooltip_title' => 'チーム名を入力してください'),
			'class' => 'form-control'
			// inputタグのクラス名
		));
        	...
        	<div class="input form-group row">
        	  ...
        	  <div class="col-sm-8">
        	    <img src="'.$cover_image_path.'" width="100%">  
						...
						<div class="col-sm-2">
        	    '.$this->Form->postLink(__('削除'), ['action' => 'deleteCoverImage',
					$team->id], ['block' => true,
					'confirm' => __('本当にカバー画像を削除してもよろしいでしょうか?'),
        	...
        	<div class="input form-group row">
        	  ...
        	  <div class="col-sm-8">
        	    <img src="'.$icon_image_path.'" width="100%">  
						...
						<div class="col-sm-2">
        	    '.$this->Form->postLink(__('削除'), ['action' => 'deleteIcon',
					$team->id], ['block' => true,
					'confirm' => __('本当にアイコンを削除してもよろしいでしょうか?'),
					'class' => 'btn btn-danger']).'
        	  </div>
        	</div>
        	';
