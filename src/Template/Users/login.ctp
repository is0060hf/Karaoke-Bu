...
$form_template = array('error' => '<div class="error-message alert alert-danger">{{content}}</div>',
...
<?= $this->Form->input('login_name', array('label' => array('text' => 'ログイン名',
	// labelで出力するテキスト
...
<?= $this->Form->input('password', array('label' => array('text' => 'パスワード',
	// labelで出力するテキスト
	'class' => 'col-form-label'
	// labelタグのクラス名
),
