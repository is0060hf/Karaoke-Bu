...
<div class="users form large-9 medium-8 columns content">
	...
	<?= $this->Form->create($store, array(
		'templates' => $form_template
	)); ?>
	<fieldset>
		<legend><?= __('店舗情報編集') ?></legend>
		<?php
		echo $this->Form->control('store_name', array(
			'label' => array(
				'text' => '店舗名',       // labelで出力するテキスト
		...
		echo $this->Form->control('store_url', array(
			'label' => array(
				'text' => 'サイトURL',       // labelで出力するテキスト
				'class' => 'col-form-label' // labelタグのクラス名
			),
			'type' => 'text',
			'templateVars' => array(
