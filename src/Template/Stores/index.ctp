...
<div class="row">
	...
	<div class="col-6 text-right">
		<a href="<?= $this->Url->build(['controller' => 'Stores', 'action' => 'add']); ?>" class="btn btn-success mt-2">
		...
		<?php foreach ($stores as $store): ?>
			<tr>
				<td class="align-middle"><a
						href="<?php echo $this->Url->build(['controller' => 'Stores', 'action' => 'view', $store->id]); ?>"
				...
				<td class="align-middle actions">
					<?= $this->Html->link(__('編集'), ['action' => 'edit', $store->id]) ?>
					<?= $this->Form->postLink(__('削除'), ['action' => 'delete', $store->id], ['confirm' => __('本当に削除してもよろしいでしょうか # {0}?', $store->id)]) ?>
				</td>
			</tr>
		<?php endforeach; ?>
		</tbody>
